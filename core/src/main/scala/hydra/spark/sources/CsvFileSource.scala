 ...
 */
case class CsvFileSource(val path: String, val properties: Map[String, Any]) extends AbstractFileSource[LongWritable, Text, TextInputFormat] {

  override def createStream(sc: StreamingContext): DStream[Text] = {
    setHadoopConfig(sc.sparkContext)
    val stream = sc.fileStream[LongWritable, Text, TextInputFormat](path, defaultFilter, false).map {
      case (k, v) => v
    }.filter(_ != null)
