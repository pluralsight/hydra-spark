...
trait DataFrameWriterExtensions extends Logging {
  ...
  implicit class Upsert(w: DataFrameWriter[Row]) {
    def upsert(idColumn: Option[StructField], jdbcOptions: JDBCOptions, df: DataFrame): Unit = {
      ...
      idColumn match {
        case Some(id) => doUpsert(df, idColumn, jdbcOptions)
        case None => JdbcUtils.saveTable(df, url, table, jdbcOptions)
      }
    }
  }

  private def doUpsert(df: DataFrame, idCol: Option[StructField], jdbcOptions: JDBCOptions) {
    ...
    df.foreachPartition { iterator =>
      val table = jdbcOptions.table
      upsertPartition(getConnection, table, iterator, idCol, rddSchema, nullTypes, jdbcOptions.batchSize, dialect)
    }
  }
