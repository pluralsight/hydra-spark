...
class DatabaseUpsertSpec extends Matchers with FunSpecLike with ScalaFutures with PatienceConfiguration
  ...
  describe("The DatabaseUpsert Transform") {
    ...
    it("collects metrics on upserts") {
      ...
      trans.run()
      val sourceName = s"${sc.appName}.${trans.name}"
      HydraMetrics.counterValue(sourceName, s"${db.id}::processedRows").get shouldBe 2

      eventually {
        HydraMetrics.getOperationMetrics(db.id)(0) match {
