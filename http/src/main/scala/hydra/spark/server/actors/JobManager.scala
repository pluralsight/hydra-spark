...
import hydra.spark.server.util.JobUtils
...
/**
  ...
  * }}}
  */
abstract class JobManager(contextConfig: Config, daoActor: ActorRef) extends Actor {

  val logger = Logging(this)

  val config = context.system.settings.config
  private val maxRunningJobs = JobUtils.getMaxRunningJobs(config)
...
//
//
//  private def getEnvironment(jId: String): JobEnvironment = {
//    val ctxCfg = contextConfig
//    new JobEnvironment {
//      val jobId: String = jId
//      val contextConfig: Config = ctxCfg
//    }
//  }
//
...
object JobManager {
  ...
  def props(contextConfig: Config, daoActor: ActorRef): Props = Props(classOf[JobManager],
