...
import hydra.spark.server.job.{BinaryInfo, BinaryType, JobInfo, JobStatus}
...
class JobSqlDAO(config: Config) extends JobRepository with FileCacher {
  ...
  val db = if (enableDbcp) {
    ...
    Database.forDataSource(dataSource, Some(dbcpMaxActive))
  } else {
  ...
  override def getJobInfos(limit: Int, statusOpt: Option[JobStatus] = None): Future[Seq[JobInfo]] = {
